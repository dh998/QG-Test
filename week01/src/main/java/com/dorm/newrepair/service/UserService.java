...
public  class UserService {
    ...
    public boolean updatePassword(String account, String oldPassword, String newPassword) {
        try (SqlSession session = MybatisUtil.getSqlSession()) {
            ...
            if (user == null || !user.getPassword().equals(oldPassword)) {
                ...
                //如果正确 就把密码改成输入的新密码
                return mapper.updatePassword(account, newPassword) > 0;
            }
        }
    }

    //学生绑定/修改宿舍
    public static boolean updateDorm(String account, String building, String roomNum) {
        try (SqlSession session = MybatisUtil.getSqlSession()) {
            UserMapper mapper = session.getMapper(UserMapper.class);
            int rows = mapper.updateDorm(account, building, roomNum);
            session.commit();
            return rows > 0;
        }
    }
    ...
    //内部验证学生还是员工
    private static boolean checkAccount(String account, Integer role) {
